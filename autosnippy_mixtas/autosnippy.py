...
def main():
    ...
    out_trimm_dir= os.path.join(output, 'trimmed_seqs')
    ...
    for r1_file, r2_file in zip(r1, r2):
        ...
        if sample in sample_list_F:
            ...
            if not os.path.isfile(output_final_vcf):
                ...
                TODO: Human filter
                """
                # USE FASTP TO TRIM SEQS
                ###################################################
                r1_trim, r2_trim = fastp_trimming(r1_file, r2_file, sample, out_trimm_dir, threads=6, min_qual=20, window_size=10, min_len=35)
                ...
                if os.path.isfile(output_vcf_sub) and os.path.isfile(output_vcf):
                    ...
                    run_snippy(r1_trim, r2_trim, reference, out_variant_dir, sample,
